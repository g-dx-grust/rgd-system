...
import { listInvoices, INVOICE_STATUS_LABELS } from "@/server/repositories/invoices";
  ...
  return (
    <div className="space-y-5">
      ...
      <h1 className="text-[22px] font-semibold text-[var(--color-text)]">
        ...
        <span className="ml-2 text-base font-normal text-[var(--color-text-muted)]">請求管理</span>
      </h1>

      {/* タブナビ */}
      <CaseTabNav caseId={caseId} activeTab="billing" />
      ...
      <section className="border border-[var(--color-border)] rounded-[var(--radius-md)] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 bg-[var(--color-bg-secondary)] border-b border-[var(--color-border)]">
          <h2 className="text-base font-semibold text-[var(--color-text)]">請求一覧</h2>
        </div>
        ...
        {invoices.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-[var(--color-bg-secondary)] border-b border-[var(--color-border)]">
              <tr>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[var(--color-text-sub)]">請求番号</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[var(--color-text-sub)]">請求日</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[var(--color-text-sub)]">支払期限</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[var(--color-text-sub)]">金額</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-[var(--color-text-sub)]">ステータス</th>
                {canEdit && (
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {invoices.map((inv) => (
                <InvoiceRow key={inv.id} invoice={inv} caseId={caseId} canEdit={canEdit} />
              ))}
            </tbody>
          </table>
        )}
      ...
      {/* 新規請求作成フォーム */}
      {canEdit && (
        <BillingFormClient caseId={caseId} />
      )}
    </div>
  );
}
...
function SummaryCard({
  label, value, accent, success, error,
}: {
  label: string; value: number;
  accent?: boolean; success?: boolean; error?: boolean;
}) {
  const textColor = error ? "text-[var(--color-error)]"
    : success ? "text-[#16A34A]"
...
function InvoiceRow({
  ...
  const statusColor =
    invoice.billingStatus === "paid"      ? "text-[#16A34A]" :
    invoice.billingStatus === "overdue"   ? "text-[var(--color-error)]" :
    invoice.billingStatus === "sent"      ? "text-[var(--color-accent)]" :
    invoice.billingStatus === "cancelled" ? "text-[var(--color-text-muted)]" :
    "text-[var(--color-text-sub)]";

  const isOverdue = invoice.dueDate && new Date(invoice.dueDate) < new Date() && invoice.billingStatus === "sent";
...
// ---------------------------------------------------------------
function InvoiceStatusActions({ invoice, caseId }: { invoice: InvoiceRow; caseId: string }) {
  ...
  if (nextActions.length === 0) return <span className="text-xs text-[var(--color-text-muted)]">—</span>;

  return (
    <div className="flex items-center gap-2 flex-wrap">
      {nextActions.map((act) => (
        <form key={act.status} action={updateInvoiceStatusAction.bind(null, null) as unknown as (fd: FormData) => Promise<void>}>
